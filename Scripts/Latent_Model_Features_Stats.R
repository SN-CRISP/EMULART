...
################################################################################
encoder_wu_path <- paste0("~/Desktop/SKIRT-VDAE-INLA/Training_Results/VDAE_",  #
                          "Model/32-16-8-16-32_ep=4341_bias_0.95_pat=500_actf",#
                          "=selu_lf=custom_optf=adam_bs=32_wZeros_ENCODER_U_", #
                          "WEIGHTS.h5")                                        #
encoder_ws_path <- paste0("~/Desktop/SKIRT-VDAE-INLA/Training_Results/VDAE_",  #
                          "Model/32-16-8-16-32_ep=4341_bias_0.95_pat=500_actf",#
                          "=selu_lf=custom_optf=adam_bs=32_wZeros_ENCODER_S_", #
                          "WEIGHTS.h5")                                        #
decoder_w_path <- paste0("~/Desktop/SKIRT-VDAE-INLA/Training_Results/VDAE_",   #
                         "Model/32-16-8-16-32_ep=4341_bias_0.95_pat=500_actf", #
                         "=selu_lf=custom_optf=adam_bs=32_wZeros_DECODER_",    #
                         "WEIGHTS.h5")                                         #
################################################################################
...
#### To hold positions of spaxels with spectral values == 0 ####
################################################################
null_spx <- array_reshape(true_Null, dim = spatial_size * mult)#
null_array_indeces <- which(null_spx == TRUE, arr.ind = TRUE)  #
################################################################
...
sample_null_indeces <- sample(which(null_spx == TRUE), null_amount)          #
input_data_indeces <- c(non_null_array_indeces, sample_null_indeces)         #
input_dataset_size <- length(input_data_indeces)                             #
rm(non_null_array_indeces, null_array_indeces, sample_null_indeces, null_spx)#
                                                                             #
...
#########################################################################
encoded_u_xtest <- encoder_mean %>% predict(xtest)                      #
encoded_s_xtest <- encoder_var %>% predict(xtest)                       #
...
# into an array for which regular R operations are available            #
...
##########################################################################
######## Creating Corner Plots for Training Data Encoded Features ########
##########################################################################
target_path <- "~/Desktop/SKIRT-VDAE-INLA/Training_Results/VDAE_Model/"  #
pdf(paste0(target_path,'_corner_plots.pdf'), width = 21, height = 29.7)  #
                                                                         #
par(cex.main = 1.5)                                                      #
if(latent_size == 1){                                                    #
  par(mfrow = c(2, 1))                                                   #
  ...
  switch(log(latent_size, 2),                                            #
         ...
         pairs(~Z1 + Z2 + Z3 + Z4, data = encoded_xtest_df,              #
               upper.panel = NULL, cex = 2),                             #
         pairs(~Z1 + Z2 + Z3 + Z4 + Z5 + Z6 + Z7 + Z8,                   #
               data = encoded_xtest_df, upper.panel = NULL, cex.axis = 2))
}                                                                        #
dev.off()                                                                #
##########################################################################

######################################################################
##### Calculating and Saving Pearson Correlation Coefficients of #####
    ...
    if(i == j){                                                      #
      pcc_m[i, j] <- 1                                               #
                                   ...
                                   as.matrix(encoded_xtest_df[j]))   #
                                                                     ...
                                                                     #
write.table(pcc_m, file = paste0(target_path, "pearsonCC_table.csv"))#
######################################################################